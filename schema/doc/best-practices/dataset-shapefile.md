...
Below is a suggested subset of Schema.Org properties that will augment the `associatedMedia` class, derived from the [core metadata](core.md), to describe a shapefile. The file-type specific properties are encoded as `1` or `1+` to denote **required**, and `0,1` or `0+` to indicate **optional**, as illustrated in the Cardinality column in the table below. 
